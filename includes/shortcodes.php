...
function tg_social_icons_func($atts, $content) {
	...
	{
		$return_html.='<li class="flickr"><a target="_blank" title="Flickr" href="http://flickr.com/people/'.esc_url($pp_flickr_username).'"><i class="fa fa-flickr"></i></a></li>';
	}
	...
	{
		$return_html.='<li class="500px"><a target="_blank" title="500px" href="http://behance.net/'.$pp_500px_url.'"><i class="fa fa-500px"></i></a></li>';
	}
...
function tg_social_share_func($atts, $content) {
	...
	$return_html.='<h5>'.esc_html__( 'Share On', 'grandportfolio-custom-post' ).'</h5><br/><br/>';
	$return_html.='<ul>';
...
function tg_map_func($atts) {
	...
	$map_data = unserialize(stripslashes($ext_attr_serialize));
	$marker = '{ ';
	$marker.= 'MapOptions: { ';
	...
	$return_html.= '<script>';
	$return_html.= '
		jQuery(document).ready(function(){ jQuery("#'.esc_js($map_data['id']).'").simplegmaps('.stripslashes($marker).'); });
	';
	$return_html.= '</script>';
...
function tg_grid_gallery_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		...
		foreach($images_arr as $key => $image)
		{
			if ($items == 0 OR $i < $items)
			{
				...
				$small_image_url = wp_get_attachment_image_src($image, 'grandportfolio-gallery-thumb', true);
		...
		if($items > 0)
		{
			...
			$return_html.= '<script>';
			$return_html.='
				function loadGalleryImage'.esc_js($custom_id).'()
				{
					if(jQuery("#'.esc_js($custom_id).'_status").val() == 0)
					{
						...
						jQuery.ajax({
							...
							type:"POST",
							data: "action=grandportfolio_gallery_grid&gallery_id='.esc_js($gallery_id).'&items='.esc_js($items).'&offset="+currentOffset+"&columns='. esc_js($columns).'&type=grid&tg_security='.$params['ajax_nonce'].'",
							success: function(html)
							{
								jQuery("#'.esc_js($custom_id).'_offset").val(parseInt(currentOffset+'.esc_js($items).'));

								var newItems = jQuery(html);
								jQuery("#'.esc_js($custom_id).'").isotope( "insert", newItems );

								jQuery("#'.esc_js($custom_id).'").imagesLoaded(function(){
									jQuery("#'.esc_js($custom_id).'").children(".element").children(".gallery_type").each(function(){
				...
				jQuery(window).load(function(){
					...
					if (jQuery(document).height() <= jQuery(window).height())
					{
						...
						{
							loadGalleryImage'.esc_js($custom_id).'();
						}
					}
					...
					jQuery(window).on("scroll", function() {
						...
						jQuery("#'.esc_js($custom_id).'").imagesLoaded(function(){
							wrapperHeight = jQuery(this).height()+jQuery(this).offset().top;
						});

						if(jQuery(window).scrollTop() >= jQuery(document).height() - wrapperHeight)
						{
							if (currentOffset >= total)
							{
								return false;
	...
	{
		$return_html.= esc_html__( 'Empty gallery item. Please make sure you have upload image to it or check the short code.', 'grandportfolio-custom-post' );
	}
...
function tg_masonry_gallery_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		...
		foreach($images_arr as $key => $image)
		{
			...
			{
				$image_url = wp_get_attachment_image_src($image, 'original', true);
				$small_image_url = wp_get_attachment_image_src($image, 'grandportfolio-gallery-masonry', true);
				...
				function loadGalleryImage'.esc_js($custom_id).'()
				{
					if(jQuery("#'.esc_js($custom_id).'_status").val() == 0)
					{
						...
						jQuery.ajax({
							url: "'.esc_url($params['ajaxurl']).'",
							type:"POST",
							data: "action=grandportfolio_gallery_grid&gallery_id='.esc_js($gallery_id).'&items='.esc_js($items).'&offset="+currentOffset+"&columns='. esc_js($columns).'&type=masonry&tg_security='.$params['ajax_nonce'].'",
							success: function(html)
							{
								jQuery("#'.esc_js($custom_id).'_offset").val(parseInt(currentOffset+'.esc_js($items).'));
...
function tg_teaser_func($atts, $content) {
	...
	if(!empty($image))
	{
		...
		$image_id = grandportfolio_get_image_id($image);
...
function tg_grid_portfolio_func($atts, $content) {
					...
					case 'Vimeo Video':
						...
						$return_html.= '<div style="display:none;">
							<div id="video_'.$portfolio_video_id.'" style="width:900px;height:506px" class="video-container">
								...
								<iframe src="https://player.vimeo.com/video/'.$portfolio_video_id.'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" width="900" height="506" frameborder="0" webkitallowfullscreen="" mozallowfullscreen="" allowfullscreen=""></iframe>
	...
	{
		$return_html.= esc_html__( 'Empty portfolio item. Please make sure you have created portfolio item or check the short code.', 'grandportfolio-custom-post' );
	}

	$return_html.= '<br class="clear"/>';
	wp_enqueue_script("grandportfolio-script-portfolio-shortcode-".$custom_id, get_template_directory_uri()."/templates/script-portfolio-shortcode.php?id=".$custom_id, false, THEMEVERSION, true);
...
function tg_filter_portfolio_func($atts, $content) {
	...
	if(!empty($sets_arr) && empty($term))
	{
		$return_html.= '<ul class="portfolio_filters portfolio-main filter shortcode">
			<li class="all-projects active">
				<a data-target="'.esc_attr($portfolio_wrapper_id).'" class="active" href="javascript:;" data-filter="*">'.esc_html__( 'All', 'grandportfolio-custom-post' ).'</a>
	...
	$return_html.= '<br class="clear"/>';
	wp_enqueue_script("grandportfolio-script-portfolio-shortcode-".$portfolio_wrapper_id, get_template_directory_uri()."/templates/script-portfolio-shortcode.php?id=".$portfolio_wrapper_id, false, THEMEVERSION, true);
...
function tg_tab_func($atts, $content) {
	...
	wp_enqueue_script('grandportfolio-custom-tab', get_template_directory_uri()."/js/custom-tab.js", false, THEMEVERSION, true);
...
function tg_accordion_func($atts, $content) {
	...
	wp_enqueue_script('grandportfolio-custom-accordion', get_template_directory_uri()."/js/custom-accordion.js", false, THEMEVERSION, true);
...
function tg_divider_func($atts, $content) {
	...
	if($style == 'totop')
	{
		$return_html.= '<a class="hr_totop" href="#">'.esc_html__( 'Go to top', 'grandportfolio-custom-post' ).'&nbsp;<i class="fa fa-arrow-up"></i></a>';
	}
		...
		foreach($team_arr as $key => $member)
		{
			...
			if(!empty($small_image_url[0]))
			{
				...
				if(!empty($member_facebook) OR !empty($member_twitter) OR !empty($member_google) OR !empty($member_linkedin))
				{
					...
					$return_html.= '<div class="social_follow">'.esc_html__( 'Follow', 'grandportfolio-custom-post' ).'</div><ul class="social_wrapper team">';
					...
					{
						$return_html.= '<li><a title="'.$member->post_title.' on Facebook" target="_blank" class="tooltip" href="http://facebook.com/'.$member_facebook.'"><i class="fa fa-fa-facebook-official"></i></a></li>';
					}
...
function tg_testimonial_slider_func($atts, $content) {
	...
	wp_enqueue_script("grandportfolio-script-testimonials-flexslider", get_template_directory_uri()."/js/script/script-testimonials-flexslider.js", false, THEMEVERSION, true);
...
function tg_lightbox_func($atts, $content) {
	...
	if(!empty($vimeo_id))
	{
		$return_html.= '<div style="display:none;"><div id="video_'.$vimeo_id.'" class="video-container"><iframe src="https://player.vimeo.com/video/'.$vimeo_id.'?title=0&amp;byline=0&amp;portrait=0" width="900" height="506" frameborder="0" webkitallowfullscreen="" mozallowfullscreen="" allowfullscreen=""></iframe></div></div>';
	}
...
function tg_animate_counter_func($atts, $content) {
	...
	wp_enqueue_script("grandportfolio-odometer-js", get_template_directory_uri()."/js/odometer.min.js", false, THEMEVERSION, true);
	...
	setTimeout(function(){
		jQuery("#'.$custom_id.'").html('.esc_js($end).');
	}, 1000);
	';
	$return_html.= '</script>';
...
function tg_animate_circle_func($atts, $content) {
	...
	wp_enqueue_style("grandportfolio-jquery-circliful", get_template_directory_uri()."/css/jquery.circliful.css", false, THEMEVERSION, "all");
	wp_enqueue_script("grandportfolio-jquery-circliful", get_template_directory_uri()."/js/jquery.circliful.min.js", false, THEMEVERSION, true);
	wp_enqueue_script("grandportfolio-script-animate-counter", get_template_directory_uri()."/js/script/script-animate-circle-shortcode.js", false, THEMEVERSION, true);

	$return_html = '
				<div class="visual_circle">
					<div id="'.$custom_id.'" class="animate_circle" data-dimension="'.esc_attr($dimension).'" data-width="'.esc_attr($width).'" data-percent="'.esc_attr($percent).'" data-fgcolor="'.esc_attr($color).'" data-bgcolor="#f0f0f0" data-text="'.esc_attr($content).'" data-fontsize="'.esc_attr($fontsize).'" data-info="'.esc_attr($subject).'"></div>';
...
function tg_gallery_slider_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		...
		$return_html.='
			jQuery(window).load(function(){
				jQuery("#'.$custom_id.'").flexslider({
					  animation: "fade",
					  animationLoop: true,
					  itemMargin: 0,
					  minItems: 1,
					  maxItems: 1,
					  slideshow: '.esc_js($autoplay).',
					  controlNav: false,
					  smoothHeight: true,
					  slideshowSpeed: '.intval($timer*1000).',
					  animationSpeed: 1000,
					  move: 1
				});
			});
...
function tg_audio_func($atts, $content) {
	...
	wp_enqueue_script("mediaelement-and-player", get_template_directory_uri()."/js/mediaelement/mediaelement-and-player.min.js", false, THEMEVERSION);
	...
	jQuery(document).ready(function(){
		var player = new MediaElementPlayer("#'.esc_js($custom_id).'", {
...
function tg_jwplayer_func($atts, $content) {
	...
	$return_html.= '
	jQuery(document).ready(function(){
		jwplayer("'.esc_js($id).'").setup({
			flashplayer: "'.get_template_directory_uri().'/js/player.swf",
			file: "'.esc_js($file).'",
			image: "'.esc_js($image).'",
			width: '.esc_js($width).',
...
function googlefont_func($atts, $content) {
	...
	if(!empty($font))
	{
		...
		wp_enqueue_style('grandportfolio-'.$encoded_font, "https://fonts.googleapis.com/css?family=".$encoded_font, false, "", "all");
